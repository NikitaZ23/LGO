...
Чекбокс `Показать с текстурой` в заголовке результата показывает `textured_mesh.glb` поверх `white_mesh.glb`, если обе версии есть.
Так белая базовая геометрия остается подложкой и прикрывает мелкие пустоты текстурированного remesh.
Это режим просмотра: камеры двух слоев синхронизируются, но идеальное совпадение зависит от того, насколько remesh сохранил форму исходной белой модели.
