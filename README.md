...
Фронт не накладывает эти модели друг на друга: текстурный проход делает свой remesh, поэтому формы могут немного отличаться.
Если текстурная версия заметно теряет форму, заново запустите `Add texture` с `Balanced` или `High texture`.
